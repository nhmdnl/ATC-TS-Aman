...
| Radar canvas blank/white, rest of UI fine | WebGL unavailable — GPU process crashed and Chromium fell back to software (console shows "Failed to create WebGPU Context Provider", terminal shows `exit_code=139`). On Linux the bundled ANGLE's GL backend segfaults in `EGL_CreateWindowSurface` against current mesa; `electron/main.ts` pins Linux to X11 + ANGLE-on-Vulkan and gates window creation on `getGPUFeatureStatus().webgl` (2026-07-18) |
...
4. **Gate assignment — fixed 2026-07-11, taxi-in cut 2026-07-16.** Arrivals take the first *free* gate at end of ROLLOUT and mark it in `gameState.occupiedGateIds` (freed by `removeAircraft`). All-gates-occupied falls back to double-parking at gate 1 (`ponytail:` in `phase-transitions.ts`). Since 2026-07-16 the aircraft **teleports** to the gate (user decision — routed taxi-in stalled at the runway edge); `TAXI_IN` is unreachable until visible taxi-in returns.
